...
public:
	typedef typename _BaseClass::handle_type handle_type;
public:
	CSincroObjectT()
	{
		//
...
/*
inline BOOL CreateProcessAndWait(LPCTSTR file,LPTSTR params,
								 LPCTSTR CurDir = NULL)
{
	...
	if(res && proc_info.hProcess != INVALID_HANDLE_VALUE){
		...
		proc.WaitWithMsgLoop();
	}
	else
		::MessageBox(NULL,_T("Could not launch ps2.exe"),_T("Error"),MB_OK);
	return res;
}
*/
