...
#ifndef __ATLSTR_H__
	#error winfile.h requires atlstr.h
#endif //__ATLMISC_H__
	...
	BOOL ReadAll(CDataBuffer<ch> &db,int ChunkSize = 2048)
	{
		...
		if(IsHandleValid()){
			...
			if(buf){
				...
				do{
					::ZeroMemory(buf,ChunkSize * sizeof(ch));
					res = Read(buf, ChunkSize,&dwRead);
					ATLASSERT(res);
					dwTotal += dwRead;
					db.Add((LPBYTE)buf,dwRead * sizeof(ch));
				}while(dwRead == ChunkSize && res && dwRead);
